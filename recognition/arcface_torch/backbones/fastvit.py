    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.feature = nn.Sequential(
            nn.Linear(
                in_features=int(embed_dims[-1] * cls_ratio) * self.fc_scale,
                out_features=embed_dims[-1],
                bias=False,
            ),
            nn.BatchNorm1d(num_features=embed_dims[-1], eps=2e-5),
        )
...
def get_model(name: str, **kwargs) -> FastViT:
    # get model from name defined in this module
    # initialize with default arguments
    model_def_function = globals().get(name)
    if model_def_function is None:
        raise RuntimeError("Unknown model (%s)" % name)
    return model_def_function(**kwargs)
